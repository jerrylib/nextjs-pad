...
// === Components === //
import { Button, Collapse, Table, Space, Row, Col, Input, Modal, List } from 'antd'
import { CloudDownloadOutlined, CheckCircleOutlined, SyncOutlined, LoadingOutlined } from '@ant-design/icons'
import { Transaction } from '../components'

// === Reducer === //
import { Web3Context } from '../reducer/web3Reducer'
import { WEB3_INIT, UPDATE_BLOCK_NUMBER, LOAD_MORE, UPDATE_BLOCK_DETAIL, UPDATE_HASH_DETAIL } from '../reducer/actions'
...
const columns = [
  ...
  },
  {
    ...
    key: 'decodeData',
    render: v => {
      return v === undefined ? <CloudDownloadOutlined /> : <CheckCircleOutlined />
    }
  }
]
function Scan() {
  ...
  const { web3Instance, blockList = [], blockNumber, currentBlock, currentHash } = state
  ...
  const loadDetail = async (blockNumber = 0) => {
    if (blockNumber <= 0 || isNaN(blockNumber)) return
    const details = await web3Instance.eth.getBlock(`${blockNumber}`).then(blockDetail => {
      const { transactions } = blockDetail
      const transactionRequestArray = map(transactions, web3Instance.eth.getTransaction)
  ...
  const loadTransactionDetails = async (blockNumber, transactionHash) => {
    Promise.all([web3Instance.eth.getTransaction(transactionHash), web3Instance.eth.getTransactionReceipt(transactionHash)])
      .then(resp => {
    ...
    <Row gutter={[12, 12]} style={{ padding: 24 }}>
      ...
      <Col span={24}>
        <p>
          ...
          <span style={{ fontWeight: 'bold', margin: '0 0.5rem' }}>{state.blockNumber}</span>
          {loading ? <LoadingOutlined /> : <SyncOutlined onClick={fetchNewestBlockNumber} />}
        </p>
      </Col>
      <Col span={24}>
        <Collapse accordion activeKey={currentBlock} onChange={v => loadDetail(1 * v)}>
                  ...
                  columns={[
                    ...columns,
                    {
                      ...
                      render: v => {
                        return (
                          <Space>
                            <Button type="link" size="small" onClick={() => loadTransactionDetails(id, v)}>
                              Detail
                            </Button>
                          </Space>
                        )
      ...
      <Modal title="Transation Details" visible={openModal} width="100%" onOk={() => setOpenModal(false)} onCancel={() => setOpenModal(false)}>
        <p>Hash: {currentTransationItem?.hash}</p>
        {currentTransationItem === undefined || currentTransationItem.decodeData === undefined ? (
          <p>解析失败</p>
        ) : (
          <List
            size="small"
